        ...
        if (e instanceof Array) {
          if (e.length) {
            for (var d = 0, a = e.length; d < a; d++) {
              c.push(b, "=", encodeURIComponent(e[d] === undefined
                ? ""
                : e[d]), "&")
            }
          } else {
    ...
    if (b) {
      ...
      if (b.xmlData) {
        ...
        if (b.jsonData) {
          ...
          g = "POST";
          f = typeof b.jsonData == "object"
            ? Ext.encode(b.jsonData)
            : b.jsonData
        }
      }
    }
          ...
          case "select-multiple":
            for (var e = 0; e < c.options.length; e++) {
              if (c.options[e].selected) {
                if (Ext.isIE) {
                  g += encodeURIComponent(a) + "=" + encodeURIComponent(c.options[e].attributes.value.specified
                    ? c.options[e].value
                    : c.options[e].text) + "&"
                } else {
                  g += encodeURIComponent(a) + "=" + encodeURIComponent(c.options[e].hasAttribute("value")
                    ? c.options[e].value
                    : c.options[e].text) + "&"
                }
              }
  ...
  handleTransactionResponse: function(f, g, a) {
    ...
    if (c >= 200 && c < 300) {
      ...
      switch (c) {
        ...
        case 12152:
        case 13030:
          b = this.createExceptionObject(f.tId, g.argument, (a
            ? a
            : false));
          if (g.failure) {
            if (!g.scope) {
...
Sbi.sdk.apply(Sbi.sdk.services, {
  ...
  initServices: function() {
    ...
    this.services.execute = {
      ...
      baseParams: {
        NEW_SESSION: "TRUE",
        MODALITY: "SINGLE_OBJECT_EXECUTION_MODALITY",
        IGNORE_SUBOBJECTS_VIEWPOINTS_SNAPSHOTS: "true"
      }
    };
  ...
  getServiceUrl: function(a, c) {
    ...
    if (this.services[a] === undefined) {
      ...
      if (this.services[a].type === "PAGE") {
        e = {
          PAGE: this.services[a].name
        }
      } else {
        e = {
          ACTION_NAME: this.services[a].name
        }
      }
...
Sbi.sdk.apply(Sbi.sdk.api, {
  ...
  getDocumentUrl: function(a) {
    ...
    if (a.displaySliders !== undefined) {
      c.SLIDERS_VISIBLE = a.displaySliders
    }
    if (a.theme !== undefined) {
      c.theme = a.theme
    }
    if (a.useExtUI === true) {
      b = Sbi.sdk.services.getServiceUrl("executewithext", c)
    } else {
  ...
  getDocumentHtml: function(a) {
    ...
    if (a.iframe.style !== undefined) {
      c += ' style = "' + a.iframe.style + '" '
    }
    if (a.iframe.width !== undefined) {
      c += ' width = "' + a.iframe.width + '" '
    }
    if (a.iframe.height !== undefined) {
      c += ' height = "' + a.iframe.height + '" '
    }
    c += "></iframe>";
    return c
  },
  injectDocument: function(b) {
    ...
    b.iframe = b.iframe || {};
    b.iframe.width = c.getAttribute("width");
    b.iframe.height = c.getAttribute("height");
    c.innerHTML = this.getDocumentHtml(b)
  }
});
